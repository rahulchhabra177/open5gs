...
if [ -x /usr/bin/curl ]; then
    exec_cmd_nobail "curl -x http://proxy22.iitd.ac.in:3128 -sLf -o /dev/null 'https://deb.nodesource.com/${NODEREPO}/dists/${DISTRO}/Release'"
    RC=$?
else
...
if [ -x /usr/bin/curl ]; then
    exec_cmd "curl -x http://proxy22.iitd.ac.in:3128 -s $node_key_url | gpg --dearmor | tee $local_node_key >/dev/null"
else
...
## To install the Yarn package manager, run:
     curl -x http://proxy22.iitd.ac.in:3128 -sL $yarn_key_url | gpg --dearmor | sudo tee $local_yarn_key >/dev/null
